...
main(int argc, char **argv) {
    ...
    enum {
        rotate_x = 1000, rotate_y, rotate_z, merge, help, version,
        mirror_xy, mirror_yz, mirror_xz, scale, translate, translate_rel, center,
    ...
    while ((c = getopt_long(argc, argv, "et:i:m:nufdcvb:a:",
        ...
        switch (c) {
            ...
            case stretch:
                stretch_flag = 1;
                {
                    float *stretch_ptr[] = {&str_x_min, &str_x_max, &str_x, &str_y_min, &str_y_max, &str_y, &str_z_min,
                                            &str_z_max, &str_z};
                    int stretch_idx[10];
                    int optarg_idx;
...
redistribute it under certain conditions.  See the file COPYING for details.\n");
    ...
    if (stretch_flag) {
        printf("Stretching: X: %f:%f:%f Y:%f:%f:%f Z: %f:%f:%f\n", str_x_min, str_x_max, str_x, str_y_min, str_y_max,
               str_y, str_z_min, str_z_max, str_z);
    ...
    if (ret)
        fprintf(stderr,
                "Some part of the procedure failed, see the above log for more information about what happened.\n");
